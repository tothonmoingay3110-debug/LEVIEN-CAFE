...
export async function GET() {
  ...
  const { data, error } = await (db.from("sales_promotions" as never) as any)
    .select("id,name,badge_text,description,promotion_type,scope_type,target_ids,discount_value,buy_quantity,get_quantity,reward_product_id,reward_product_ids,reward_topping_id,minimum_subtotal,starts_on,ends_on")
