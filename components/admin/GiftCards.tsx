...
export default function GiftCards({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="adminStack giftCardAdmin">
    <section className="adminWelcome"><div><span>Stored value</span><h2>Gift Cards, without the spreadsheet.</h2><p>Issue secure codes, monitor balances and review every redemption or refund.</p></div><button className="adminPrimary" type="button" onClick={() => setShowIssueForm((current) => !current)}>{showIssueForm ? "Close form" : "+ Issue Gift Card"}</button></section>
    ...
    {showIssueForm && <section className="adminCard giftCardIssuePanel"><div className="adminCardHead"><div><span className="adminEyebrow">New secure code</span><h3>Issue Gift Card</h3></div><span className="adminHint">The full code is shown once after creation</span></div><form className="giftCardIssueForm" onSubmit={issueCard}><label>Initial balance <span>*</span><input name="amount" type="number" min="0.01" max="10000" step="0.01" placeholder="50.00" required /></label><label>Recipient name<input name="recipientName" maxLength={120} /></label><label>Recipient email<input name="recipientEmail" type="email" maxLength={254} /></label><label>Expiry date<input name="expiresOn" type="date" min={new Date().toISOString().slice(0, 10)} /></label><label className="wide">Internal note<textarea name="note" rows={3} maxLength={1000} placeholder="Optional reason, occasion, or delivery note" /></label><button className="adminPrimary wide" type="submit" disabled={issuing}>{issuing ? "Issuing…" : "Issue Gift Card"}</button></form></section>}
