...
type GiftCard = {
  ...
  createdAt: string;
  updatedAt: string;
  sale: { purchaserProfileId:string|null; purchaserEmail:string; channel: string; tenderType: string; receiptReference: string; status: string; deliveryStatus: string; paidAt: string | null } | null;
...
export default function GiftCards({ notify }: { notify: (message: string) => void }) {
  ...
  const [customers,setCustomers]=useState<Array<{id:string;firstName:string;lastName:string;email:string;phone:string;membershipNumber:string}>>([]);
  ...
  useEffect(() => {
    void loadCards();
    void fetch("/api/admin/customers",{cache:"no-store"}).then((response)=>response.json()).then((result:{profiles?:Array<{id:string;firstName:string;lastName:string;email:string;phone:string;membershipNumber:string}>})=>setCustomers(result.profiles||[])).catch(()=>setCustomers([]));
  ...
  const filteredCards = useMemo(() => {
    ...
    return cards.filter((card) => (filter === "all" || card.status === filter) && (!term || `${card.lastFour} ${card.recipientName} ${card.recipientEmail} ${card.sale?.purchaserEmail||""} ${card.note}`.toLowerCase().includes(term)));
  }, [cards, filter, query]);
    ...
    try {
      const response = await fetch("/api/admin/gift-cards", {
        ...
        body: JSON.stringify({
          ...
          purchaserProfileId: data.get("purchaserProfileId"),
      ...
      notify("Gift Card created successfully");
    } catch (issueError) {
  ...
  return <div className="adminStack giftCardAdmin">
    ...
    {showIssueForm && <section className="adminCard giftCardIssuePanel"><div className="adminCardHead"><div><span className="adminEyebrow">Verified in-store sale</span><h3>Create Gift Card</h3></div><span className="adminHint">The full code is shown once after creation</span></div><form className="giftCardIssueForm" onSubmit={issueCard}><label><span className="giftCardFieldLabel">Card value <b>*</b></span><input name="amount" type="number" min="5" max="1000" step="0.01" placeholder="50.00" required /></label><label>Payment / tender<select name="tenderType" defaultValue="card_terminal" required><option value="card_terminal">Card terminal</option><option value="cash">Cash</option><option value="complimentary">Complimentary (Owner only)</option></select></label><label><span className="giftCardFieldLabel">Receipt / terminal reference <b>*</b></span><input name="receiptReference" maxLength={120} placeholder="Receipt # or terminal reference" /></label><label>Purchaser / customer<select name="purchaserProfileId" defaultValue=""><option value="">Guest / not linked</option>{customers.map((customer)=><option value={customer.id} key={customer.id}>{customer.firstName} {customer.lastName} · {customer.membershipNumber}</option>)}</select></label><label>Guest purchaser email<input name="purchaserEmail" type="email" maxLength={254} placeholder="Used only when not linked"/></label><label>Recipient name<input name="recipientName" maxLength={120} /></label><label>Recipient email<input name="recipientEmail" type="email" maxLength={254} /></label><label>Expiry date<input name="expiresOn" type="date" min={new Date().toISOString().slice(0, 10)} /></label><label className="wide">Internal note<textarea name="note" rows={3} maxLength={1000} placeholder="Optional reason, occasion, or delivery note" /></label><button className="adminPrimary wide" type="submit" disabled={issuing}>{issuing ? "Creating…" : "Create Gift Card"}</button></form><small>Cash and terminal sales require a receipt reference. Complimentary cards are restricted to the Owner.</small></section>}
    {issuedCard && <div className="adminModalBackdrop"><section className="adminModal issuedGiftCardModal" role="dialog" aria-modal="true" aria-label="Gift Card created"><div className="issuedGiftCard"><div><span>✓ Gift Card issued successfully</span><h3>{issuedCard.code}</h3><p>Balance {money(issuedCard.initialBalance)} · Ending {issuedCard.lastFour}. This full code cannot be recovered after closing.</p></div><div><button className="adminPrimary" type="button" onClick={() => void copyCode()}>Copy code</button><button className="adminSecondary" type="button" onClick={() => setIssuedCard(null)}>Create another</button></div></div></section></div>}
