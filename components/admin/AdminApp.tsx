...
type Product = {
  id: string; sku: string; name: string; categoryId: string; price: number; description: string; image: string; emoji: string;
...
function AdminModal({ modal, db, close, update }: { modal: { type: string; id?: string }; db: DB; close: () => void; update: (d: DB, m?: string) => void }) {
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if(modal.type==="product") { const old=entity as Product|undefined; const categoryId=String(f.get("categoryId")||""); const sku=old?.sku||generatedProductSku(db.products,db.categories,categoryId); if(db.products.some((product)=>product.id!==old?.id&&product.sku.toUpperCase()===sku)){update(db,"SKU already exists");return;} const item:Product={id:old?.id||catalogId(),sku,name:String(f.get("name")),categoryId,price:Number(f.get("price")),description:String(f.get("description")),image:image||old?.image||"",emoji:old?.emoji||"☕",toppingIds:f.getAll("toppings").map(String),allowIce:f.get("allowIce")==="on",allowSugar:f.get("allowSugar")==="on",allowToppings:f.get("allowToppings")==="on",bestSeller:f.get("bestSeller")==="on",mustTry:f.get("mustTry")==="on",featured:f.get("featured")==="on",isNew:f.get("isNew")==="on",soldOut:f.get("soldOut")==="on",active:f.get("active")==="on"}; update({...db,products:old?db.products.map(x=>x.id===old.id?item:x):[item,...db.products]},old?"Product updated":"Product created"); }
