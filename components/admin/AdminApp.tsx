...
import QrLanding from "@/components/admin/QrLanding";
...
type MemberProfile = { id: string; authUserId: string | null; email: string; firstName: string; lastName: string; phone: string; membershipNumber: string; memberSince: string; onlineAccess: boolean };
type Employee = {
  id: string;
  email: string;
  fullName: string;
...
const seed: DB = {
  ...
  toppings: [
    { id: "t1", name: "Salted Cream", price: 1.25, image: "", active: true },
    { id: "t2", name: "Egg Cream", price: 1.25, image: "", active: true },
    { id: "t3", name: "Boba", price: 1.0, image: "", active: true },
    { id: "t4", name: "Extra Espresso Shot", price: 1.5, image: "", active: true },
  ],
  products: [
    { id: "p1", sku: "LV-COF-001", name: "Vietnamese Milk Coffee", categoryId: "c1", price: 4.99, description: "Bold coffee with condensed milk.", image: "", emoji: "☕", toppingIds: ["t1", "t4"], allowIce: true, allowSugar: true, allowToppings: true, bestSeller: true, mustTry: false, featured: true, isNew: false, soldOut: false, active: true },
    { id: "p2", sku: "LV-MT-001", name: "Brown Marble Milk Tea", categoryId: "c2", price: 5.49, description: "Brown sugar milk tea with boba.", image: "", emoji: "🧋", toppingIds: ["t3"], allowIce: true, allowSugar: true, allowToppings: true, bestSeller: true, mustTry: true, featured: false, isNew: false, soldOut: false, active: true },
    { id: "p3", sku: "LV-COF-002", name: "Ube Coffee", categoryId: "c1", price: 5.49, description: "Vietnamese coffee with sweet ube cream.", image: "", emoji: "🟣", toppingIds: ["t1", "t4"], allowIce: true, allowSugar: true, allowToppings: true, bestSeller: true, mustTry: true, featured: false, isNew: true, soldOut: false, active: true },
  ],
  combos: [
...
const viewLabels: Record<AdminView, string> = {
  dashboard: "Dashboard", orders: "Orders", memberscan: "Scan Member", messages: "Franchise Messages", eventbookings: "Event Bookings", workspace: "My Workspace", schedule: "Schedule", timeoff: "Time Off", reportcenter: "Report Center", kpidashboard: "KPI Dashboard", customeranalytics: "Customer Analytics", productanalytics: "Product Analytics", campaignanalytics: "Promotion & Combo Analytics", aiinsights: "AI Business Insights", reports: "Staff Reports", activity: "Activity Log", customers: "Customers", loyalty: "Loyalty Programs", giftcards: "Gift Cards", employees: "Employees", products: "Products", categories: "Categories",
  toppings: "Toppings", combos: "Combos", combosuggestions: "Combo Suggestions", salespromotions: "Promotions", promotions: "Homepage Content", content: "Website Content", qrcode: "QR Landing", account: "My Account",
};
...
const viewPermissions: Partial<Record<AdminView, StaffPermission>> = {
  ...
  salespromotions: "manage_catalog", promotions: "manage_catalog",
  content: "manage_catalog",
  qrcode: "manage_catalog",
};
...
const adminNavGroups: AdminNavGroup[] = [
  ...
  { id: "store", label: "Customers & Store", shortLabel: "Store", icon: "products", views: ["customers", "loyalty", "giftcards", "products", "categories", "toppings", "combos", "combosuggestions", "salespromotions", "promotions", "content", "qrcode"] },
];
...
export default function AdminApp() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      parsed.products = (parsed.products || []).map((product) => ({
        ...product,
        sku: product.sku || `LV-${product.id.slice(0, 8).toUpperCase()}`,
        allowToppings: product.allowToppings ?? (product.toppingIds || []).length > 0,
      }));
      parsed.toppings = (parsed.toppings || []).map((topping) => ({ ...topping, image: topping.image || "" }));
      setDb({ ...parsed, orders: [] });
    }
  ...
  const filteredProducts = db.products.filter(p => `${p.name} ${p.sku}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <div className="adminShellV3">
      ...
      <main className="adminWorkspace">
        <header className="adminTopbar"><div><span className="adminBreadcrumb">LEVIEN CAFE / {viewLabels[view]}</span><h1>{viewLabels[view]}</h1></div><div className="adminTopActions">{canManageOrders && <span className={`adminLiveBadge sync-${orderSyncStatus}`}>● {orderSyncStatus === "live" ? "Orders live" : orderSyncStatus === "polling" ? "Auto reconnecting" : "Connecting"}</span>}<span className={`adminRoleBadge role-${staff.role}`}>{staffRoleLabels[staff.role]}</span>{roleHasPermission(staff.role,"manage_contacts")&&<NotificationCenter openTarget={(target,id)=>{if(target==="eventbookings"){setFocusedEventBooking(id);setView("eventbookings")}}}/>}<button className="adminAvatar" title={staff.fullName} onClick={() => setView("account")}>{staff.avatarUrl ? <img src={staff.avatarUrl} alt="" /> : initials(staff.fullName)}</button></div></header>
        ...
        {view === "activity" && <ActivityLog />}
         {view === "customers" && <Customers customers={filteredCustomers} allCustomers={customers} orders={db.orders} query={customerQuery} setQuery={setCustomerQuery} openModal={setModal} notify={setToast} />}
        ...
        {view === "salespromotions" && <SalesPromotions catalog={{products:db.products,categories:db.categories,combos:db.combos,toppings:db.toppings}} notify={setToast} />}
        {view === "promotions" && <Promotions db={db} openModal={setModal} update={update} />}
        {view === "content" && <WebsiteContent db={db} openModal={setModal} />}
        {view === "qrcode" && <QrLanding notify={setToast} />}
...
function Orders({ db, orders, filter, setFilter, update, openModal }: { db: DB; orders: Order[]; filter: "All" | OrderStatus; setFilter: (v: "All" | OrderStatus) => void; update: (d: DB, m?: string) => void; openModal: (m: { type: string; id?: string }) => void }) {
  ...
  async function counterSaved(orderNumber: string) {
    ...
    if (response.ok && result.orders?.some((order) => order.id === orderNumber)) {
      openModal({ type: "order", id: orderNumber });
    }
  }
  ...
  async function changeOrderStatus(orderId: string, status: OrderStatus) {
    ...
    if (existingOrder?.status === "Completed" && status !== "Completed") {
      update(db, `Order ${orderId} is completed and locked`);
      return;
    }
...
function Customers({ allCustomers, orders, query, setQuery, openModal, notify }: { customers: Customer[]; allCustomers: Customer[]; orders: Order[]; query: string; setQuery: (value: string) => void; openModal: (modal: { type: string; id?: string }) => void; notify:(message:string)=>void }) {
  const [profiles,setProfiles]=useState<MemberProfile[]>([]); const [membershipFilter,setMembershipFilter]=useState<"all"|"member"|"guest">("all"); const [createOpen,setCreateOpen]=useState(false); const [loadingProfiles,setLoadingProfiles]=useState(true);
  const loadProfiles=useCallback(async()=>{setLoadingProfiles(true);try{const response=await fetch("/api/admin/customers",{cache:"no-store"});const result=await response.json() as {profiles?:MemberProfile[];error?:string};if(!response.ok)throw new Error(result.error||"Unable to load memberships");setProfiles(result.profiles||[])}finally{setLoadingProfiles(false)}},[]);
  useEffect(()=>{void loadProfiles()},[loadProfiles]);
  const customers=useMemo(()=>{const merged=allCustomers.map((customer)=>{const profile=profiles.find((item)=>(item.email&&customer.email&&item.email.toLowerCase()===customer.email.toLowerCase())||(normalizePhone(item.phone)&&normalizePhone(item.phone)===normalizePhone(customer.phone)));return profile?{...customer,membershipNumber:profile.membershipNumber,memberSince:profile.memberSince,onlineAccess:profile.onlineAccess}:customer});for(const profile of profiles){if(!merged.some((customer)=>customer.membershipNumber===profile.membershipNumber))merged.push({id:normalizePhone(profile.phone)||profile.id,phone:profile.phone,firstName:profile.firstName,lastName:profile.lastName,email:profile.email,totalOrders:0,totalSpent:0,averageOrder:0,firstOrderAt:profile.memberSince,lastOrderAt:profile.memberSince,orderIds:[],membershipNumber:profile.membershipNumber,memberSince:profile.memberSince,onlineAccess:profile.onlineAccess})}const term=query.trim().toLowerCase();return merged.filter((customer)=>(membershipFilter==="all"||(membershipFilter==="member")===Boolean(customer.membershipNumber))&&(!term||`${customer.firstName} ${customer.lastName} ${customer.email} ${customer.phone} ${customer.membershipNumber||""}`.toLowerCase().includes(term))).sort((left,right)=>new Date(right.lastOrderAt).getTime()-new Date(left.lastOrderAt).getTime())},[allCustomers,profiles,query,membershipFilter]);
  return <div className="adminStack">
    <section className="adminToolbar customerToolbar">
      ...
      <div className="customerToolbarActions"><div className="adminTabs">{(["all","member","guest"] as const).map((item)=><button type="button" key={item} className={membershipFilter===item?"active":""} onClick={()=>setMembershipFilter(item)}>{item==="all"?"All":item==="member"?"Members":"Guests"}</button>)}</div><button className="adminSecondary customerExportButton" type="button" onClick={() => exportCustomerPurchaseCsv(customers, orders)}>Export CSV</button><button className="adminPrimary" type="button" onClick={()=>setCreateOpen(true)}>＋ Create Customer</button></div>
    </section>
    <section className="adminCard">
      ...
      <div className="adminTableWrap"><table className="adminTable customerTable"><thead><tr><th>Customer</th><th>Membership</th><th>Phone</th><th>Products purchased</th><th>Orders</th><th>Total spent</th><th>Last order</th><th></th></tr></thead><tbody>
        {customers.map((customer) => {
          ...
          return <tr key={customer.id}>
            <td><strong>{customer.firstName} {customer.lastName}</strong><small>{customer.email || "No email"}</small></td><td>{customer.membershipNumber?<><span className="customerMemberBadge">Member</span><small>{customer.membershipNumber}</small></>:<span className="customerGuestBadge">Guest</span>}</td>
            ...
            <td>{customer.totalOrders?new Date(customer.lastOrderAt).toLocaleDateString():"—"}</td>
            <td>{customer.totalOrders?<button className="adminIconAction" onClick={() => openModal({ type: "customer", id: customer.id })}>View</button>:<span className="adminHint">No orders</span>}</td>
          </tr>;
        })}
        {!customers.length && <tr><td colSpan={8}><div className="customerEmpty"><strong>No customers found</strong><span>Change the filter or create a customer membership.</span></div></td></tr>}
      </tbody></table></div>
    </section>{loadingProfiles&&<span className="adminHint">Refreshing membership status…</span>}{createOpen&&<CreateCustomerModal close={()=>setCreateOpen(false)} saved={async(message)=>{setCreateOpen(false);await loadProfiles();setQuery("");notify(message)}}/>}
  </div>;
}

function CreateCustomerModal({close,saved}:{close:()=>void;saved:(message:string)=>Promise<void>}){const[saving,setSaving]=useState(false);const[error,setError]=useState("");async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const data=new FormData(event.currentTarget);setSaving(true);setError("");try{const response=await fetch("/api/admin/customers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({firstName:data.get("firstName"),lastName:data.get("lastName"),email:data.get("email"),phone:data.get("phone")})});const result=await response.json() as {membershipNumber?:string;invitationSent?:boolean;error?:string};if(!response.ok)throw new Error(result.error||"Unable to create customer");await saved(`${result.membershipNumber} created${result.invitationSent?" · activation email sent":" · counter-only membership"}`)}catch(submitError){setError(submitError instanceof Error?submitError.message:"Unable to create customer")}finally{setSaving(false)}}return <ModalShell title="Create customer" subtitle="Counter membership" close={close}><form className="adminForm" onSubmit={submit}><FormInput label="First name" name="firstName" defaultValue=""/><FormInput label="Last name" name="lastName" defaultValue=""/><label>Email <small>Optional for counter-only membership</small><input name="email" type="email" autoComplete="email"/></label><label>Phone <small>Required when email is empty</small><input name="phone" type="tel" autoComplete="tel"/></label><div className="adminNote wide"><strong>Customer security</strong><p>Staff never creates or sees the customer password. Customers with email receive an activation link and set their own password.</p></div>{error&&<div className="adminLoginError wide">{error}</div>}<div className="adminFormActions wide"><button className="adminSecondary" type="button" onClick={close}>Cancel</button><button className="adminPrimary" type="submit" disabled={saving}>{saving?"Creating…":"Create Customer"}</button></div></form></ModalShell>}
...
const productExportHeaders = ["SKU", "Name", "Category", "Price", "Description", "Active", "Sold Out", "Best Seller", "Must Try", "Featured", "New", "Allow Ice", "Allow Sugar", "Allow Toppings", "Available Toppings"];
function productExportRows(db: DB) { return db.products.map((product) => [product.sku, product.name, db.categories.find((category) => category.id === product.categoryId)?.name || "", product.price, product.description, product.active, product.soldOut, product.bestSeller, product.mustTry, product.featured, product.isNew, product.allowIce, product.allowSugar, product.allowToppings, product.toppingIds.map((id) => db.toppings.find((topping) => topping.id === id)?.name).filter(Boolean).join(" | ")]); }
function downloadProductFile(content: BlobPart[], type: string, extension: string) { const blob = new Blob(content, { type }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `levien-products-${new Date().toISOString().slice(0,10)}.${extension}`; link.click(); URL.revokeObjectURL(url); }
function exportProductsCsv(db: DB) { const csv = [productExportHeaders, ...productExportRows(db)].map((row) => row.map(csvCell).join(",")).join("\r\n"); downloadProductFile(["\uFEFF", csv], "text/csv;charset=utf-8", "csv"); }
function exportProductsExcel(db: DB) { const escape = (value: unknown) => String(value ?? "").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"); const table = `<table><thead><tr>${productExportHeaders.map((cell)=>`<th>${escape(cell)}</th>`).join("")}</tr></thead><tbody>${productExportRows(db).map((row)=>`<tr>${row.map((cell)=>`<td>${escape(cell)}</td>`).join("")}</tr>`).join("")}</tbody></table>`; downloadProductFile(["\uFEFF", table], "application/vnd.ms-excel;charset=utf-8", "xls"); }
...
const importBoolean = (value: string, fallback=false) => value.trim() ? ["true","1","yes","y","active"].includes(value.trim().toLowerCase()) : fallback;
...
function Products({ db, products, query, setQuery, openModal, update }: { db: DB; products: Product[]; query: string; setQuery: (v: string) => void; openModal: (m: { type: string; id?: string }) => void; update: (d: DB, m?: string) => void }) {
  ...
  const visibleProducts = products.filter((product) => categoryFilter === "all" || product.categoryId === categoryFilter);
  async function importCsv(file: File) {
    const rows = parseCsv(await file.text());
    if (rows.length < 2) { update(db, "The import file has no product rows"); return; }
    const headers = rows[0].map((header) => header.trim().toLowerCase());
    const at = (row: string[], name: string) => row[headers.indexOf(name)] || "";
    if (!headers.includes("sku") || !headers.includes("name") || !headers.includes("price")) { update(db, "CSV must include SKU, Name, and Price columns"); return; }
    const imported: Product[] = [];
    const seen = new Set<string>();
    for (const row of rows.slice(1)) {
      const sku = at(row,"sku").trim().toUpperCase(); const name = at(row,"name").trim(); const price = Number(at(row,"price"));
      if (!sku || !name || !Number.isFinite(price) || price < 0 || seen.has(sku)) { update(db, `Import stopped: invalid or duplicate SKU ${sku || "on a row"}`); return; }
      seen.add(sku);
      const old = db.products.find((product) => product.sku.toUpperCase() === sku);
      const categoryName = at(row,"category").trim().toLowerCase(); const category = db.categories.find((item) => item.name.toLowerCase() === categoryName);
      if (!category) { update(db, `Import stopped: category not found for ${sku}`); return; }
      const toppingNames = at(row,"available toppings").split("|").map((value)=>value.trim().toLowerCase()).filter(Boolean);
      const toppingIds = toppingNames.map((toppingName)=>db.toppings.find((item)=>item.name.toLowerCase()===toppingName)?.id).filter(Boolean) as string[];
      imported.push({ id:old?.id||catalogId(), sku, name, categoryId:category.id, price, description:at(row,"description").trim(), image:old?.image||"", emoji:old?.emoji||"☕", toppingIds, active:importBoolean(at(row,"active"),true), soldOut:importBoolean(at(row,"sold out")), bestSeller:importBoolean(at(row,"best seller")), mustTry:importBoolean(at(row,"must try")), featured:importBoolean(at(row,"featured")), isNew:importBoolean(at(row,"new")), allowIce:importBoolean(at(row,"allow ice")), allowSugar:importBoolean(at(row,"allow sugar")), allowToppings:importBoolean(at(row,"allow toppings"),toppingIds.length>0) });
    }
    const importedSkus = new Set(imported.map((product)=>product.sku));
    update({ ...db, products:[...imported,...db.products.filter((product)=>!importedSkus.has(product.sku))] }, `${imported.length} products imported · existing images preserved`);
  }
  return <div className="adminStack"><section className="adminToolbar productAdminToolbar"><div className="adminCatalogFilters"><div className="adminSearch"><span>⌕</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search products or SKU..." /></div><label>Category<select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="all">All categories</option>{db.categories.map((category) => <option key={category.id} value={category.id}>{category.name} ({db.products.filter((product) => product.categoryId === category.id).length})</option>)}</select></label></div><div className="productTransferActions"><input ref={importInput} type="file" accept=".csv,text/csv" hidden onChange={(event)=>{const file=event.target.files?.[0];if(file)void importCsv(file);event.currentTarget.value=""}}/><button className="adminSecondary" type="button" onClick={()=>importInput.current?.click()}>Import CSV</button><button className="adminSecondary" type="button" onClick={()=>exportProductsCsv(db)}>Export CSV</button><button className="adminSecondary" type="button" onClick={()=>exportProductsExcel(db)}>Export Excel</button><button className="adminPrimary" onClick={() => openModal({ type: "product" })}>New product</button></div></section><section className="adminCard"><div className="adminCardHead"><div><span className="adminEyebrow">Menu management</span><h3>{visibleProducts.length} of {db.products.length} products</h3></div><span className="adminHint">Search and category filters work together · Imports update by SKU</span></div><div className="adminProductList">{visibleProducts.map(p => <div className="adminProductRow" key={p.id}><div className="adminProductThumb">{p.image ? <img src={p.image} alt=""/> : <span>LV</span>}</div><div className="adminProductMain"><strong>{p.name}</strong><small>{p.sku} · {db.categories.find(c => c.id === p.categoryId)?.name || "Uncategorized"}</small></div><div className="adminProductBadges">{p.bestSeller && <span>Best Seller</span>}{p.mustTry && <span>Must Try</span>}{p.featured && <span>Featured</span>}{p.isNew && <span>New</span>}</div><strong className="adminProductPrice">{money(p.price)}</strong><span className={p.soldOut ? "adminState sold" : p.active ? "adminState live" : "adminState hidden"}>{p.soldOut ? "Sold out" : p.active ? "Live" : "Hidden"}</span><div className="adminRowActions"><button onClick={() => openModal({ type: "product", id: p.id })}>Edit</button><button onClick={() => update({ ...db, products: db.products.map(item => item.id === p.id ? { ...item, active: !item.active } : item) }, p.active ? "Product hidden" : "Product published")}>{p.active ? "Hide" : "Show"}</button></div></div>)}{!visibleProducts.length && <div className="adminFilterEmpty">No products match the selected filters.</div>}</div></section></div>;
}

function Categories({ db, openModal, update }: { db: DB; openModal: (m: { type: string; id?: string }) => void; update: (d: DB, m?: string) => void }) {
  function moveCategory(index: number, direction: -1 | 1) {
    const destination = index + direction;
    if (destination < 0 || destination >= db.categories.length) return;
    const categories = [...db.categories];
    [categories[index], categories[destination]] = [categories[destination], categories[index]];
    update({ ...db, categories }, `Category moved ${direction < 0 ? "up" : "down"} · website order updated`);
  }
  return <section className="adminCard"><div className="adminCardHead"><div><span className="adminEyebrow">Menu structure</span><h3>Product categories</h3></div><button className="adminPrimary" onClick={() => openModal({ type: "category" })}>＋ New category</button></div><p className="categoryOrderHint">Use the arrow buttons to control the category order shown on the website and menu.</p><div className="adminCategoryList">{db.categories.map((category, index) => <div className="adminCategoryRow" key={category.id}><span className="adminEntityIcon">{category.icon}</span><div><strong>{index + 1}. {category.name}</strong><small>{db.products.filter((product) => product.categoryId === category.id).length} products</small></div><span className={category.active ? "adminState live" : "adminState hidden"}>{category.active ? "Active" : "Hidden"}</span><div className="categoryReorderActions"><button type="button" disabled={index === 0} onClick={() => moveCategory(index, -1)} aria-label={`Move ${category.name} up`} title="Move up">↑</button><button type="button" disabled={index === db.categories.length - 1} onClick={() => moveCategory(index, 1)} aria-label={`Move ${category.name} down`} title="Move down">↓</button></div><button className="adminTextButton" onClick={() => openModal({ type: "category", id: category.id })}>Edit</button><button className="adminTextButton muted" onClick={() => update({ ...db, categories: db.categories.map((item) => item.id === category.id ? { ...item, active: !item.active } : item) })}>{category.active ? "Hide" : "Show"}</button></div>)}</div></section>;
}
function Toppings({ db, openModal, update }: { db: DB; openModal: (m: { type: string; id?: string }) => void; update: (d: DB, m?: string) => void }) {
  const [query, setQuery] = useState("");
  const [status, setStatus] = useState<"all" | "active" | "inactive">("all");
  const [productId, setProductId] = useState("all");
  const linkedProducts = db.products.filter((product) => product.toppingIds.length > 0).sort((left, right) => left.name.localeCompare(right.name));
  const selectedProduct = productId === "all" ? null : db.products.find((product) => product.id === productId);
  const normalizedQuery = query.trim().toLowerCase();
  const visibleToppings = db.toppings.filter((topping) => (!normalizedQuery || topping.name.toLowerCase().includes(normalizedQuery)) && (status === "all" || (status === "active" ? topping.active : !topping.active)) && (!selectedProduct || selectedProduct.toppingIds.includes(topping.id)));
  return <div className="adminStack"><section className="adminToolbar toppingFilterToolbar"><div className="adminSearch"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search toppings..."/></div><div className="adminCatalogFilters toppingSelectFilters"><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as "all" | "active" | "inactive")}><option value="all">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label><label>Used by product<select value={productId} onChange={(event) => setProductId(event.target.value)}><option value="all">All products</option>{linkedProducts.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label></div></section><EntityPage title={`Toppings · ${visibleToppings.length} of ${db.toppings.length}`} eyebrow="Customization options" button="New topping" onAdd={() => openModal({ type: "topping" })}>{visibleToppings.map(t => { const usedBy = db.products.filter((product) => product.toppingIds.includes(t.id)).length; return <EntityRow key={t.id} icon="＋" title={t.name} subtitle={`${money(t.price)} additional · Used by ${usedBy} product${usedBy === 1 ? "" : "s"}`} active={t.active} edit={() => openModal({ type: "topping", id: t.id })} toggle={() => update({ ...db, toppings: db.toppings.map(x => x.id === t.id ? { ...x, active: !x.active } : x) })}/>})}{!visibleToppings.length && <div className="adminFilterEmpty">No toppings match the selected filters.</div>}</EntityPage></div>;
}
...
function Promotions({ db, openModal, update }: { db: DB; openModal: (m: { type: string; id?: string }) => void; update: (d: DB, m?: string) => void }) { return <EntityPage title="Homepage slider" eyebrow="Homepage content" button="New slide" onAdd={() => openModal({ type: "promotion" })}>{[...db.promotions].sort((a,b)=>a.order-b.order).map(p => <EntityRow key={p.id} icon="▣" title={p.title} subtitle={`${p.eyebrow} · Slide ${p.order} · ${p.startDate} → ${p.endDate} · ${p.priceText}`} active={promotionIsLive(p)} edit={() => openModal({ type: "promotion", id: p.id })} toggle={() => update({ ...db, promotions: db.promotions.map(x => x.id === p.id ? { ...x, active: !x.active } : x) })}/>)}</EntityPage>; }
...
function AdminModal({ modal, db, close, update }: { modal: { type: string; id?: string }; db: DB; close: () => void; update: (d: DB, m?: string) => void }) {
  ...
  const [promotionDisplayMode, setPromotionDisplayMode] = useState<"designed"|"full_image">("designed");
  ...
  useEffect(() => {
    ...
    if (modal.type === "product") setProductCategoryId((entity as Product | undefined)?.categoryId || db.categories[0]?.id || "");
    if (modal.type === "promotion") setPromotionDisplayMode((entity as Promotion | undefined)?.displayMode || "designed");
  }, [entity, modal.type]);
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if(modal.type==="product") { const old=entity as Product|undefined; const categoryId=String(f.get("categoryId")||""); const sku=old?.sku||generatedProductSku(db.products,db.categories,categoryId); if(db.products.some((product)=>product.id!==old?.id&&product.sku.toUpperCase()===sku)){update(db,"SKU already exists");return;} const item:Product={id:old?.id||catalogId(),sku,name:String(f.get("name")),vietnameseName:String(f.get("vietnameseName")||"").trim(),categoryId,price:Number(f.get("price")),description:String(f.get("description")),image:image||old?.image||"",emoji:old?.emoji||"☕",toppingIds:f.getAll("toppings").map(String),allowIce:f.get("allowIce")==="on",allowSugar:f.get("allowSugar")==="on",allowToppings:f.get("allowToppings")==="on",bestSeller:f.get("bestSeller")==="on",mustTry:f.get("mustTry")==="on",featured:f.get("featured")==="on",isNew:f.get("isNew")==="on",soldOut:f.get("soldOut")==="on",active:f.get("active")==="on"}; update({...db,products:old?db.products.map(x=>x.id===old.id?item:x):[item,...db.products]},old?"Product updated":"Product created"); }
    ...
    if(modal.type==="topping") { const old=entity as Topping|undefined; const item:Topping={id:old?.id||catalogId(),name:String(f.get("name")),price:Number(f.get("price")),image:image||old?.image||"",active:f.get("active")==="on"}; update({...db,toppings:old?db.toppings.map(x=>x.id===old.id?item:x):[...db.toppings,item]},"Topping saved"); }
    if (modal.type === "combo") {
    ...
    if(modal.type==="promotion") { const old=entity as Promotion|undefined; const startDate=String(f.get("startDate")); const endDate=String(f.get("endDate")); if(endDate<startDate){update(db,"Slide end date must be on or after its start date");return;} const full=promotionDisplayMode==="full_image"; const item:Promotion={id:old?.id||catalogId(),title:full?String(f.get("altText")||"Homepage advertisement"):String(f.get("title")),eyebrow:full?"":String(f.get("eyebrow")),description:full?"":String(f.get("description")),priceText:full?"":String(f.get("priceText")),order:Number(f.get("order")),image:image||old?.image||"",mobileImage:mobileImage||old?.mobileImage||"",displayMode:promotionDisplayMode,linkUrl:String(f.get("linkUrl")||"/menu"),active:f.get("active")==="on",startDate,endDate}; if(full&&!item.image){update(db,"Upload a desktop advertisement image");return;} update({...db,promotions:old?db.promotions.map(x=>x.id===old.id?item:x):[...db.promotions,item]},"Homepage slide saved"); }
  ...
  if(modal.type==="order") { const o=entity as Order; return <ModalShell title={o.id} subtitle="Order details" close={close}><OrderPrintActions order={o} store={{storeName:db.content.storeName,address:db.content.address,phone:db.content.phone}}/><div className="orderDetailHeader"><div><strong>{o.customer}</strong><span>{o.phone} · {o.type}</span></div><b>{money(o.total)}</b></div>{o.type === "Pickup" && <div className="adminPickupNotice"><span>Customer pickup time</span><strong>{formatPickupTime(o.pickupTime)}</strong></div>}<div className="orderItemList">{o.items.map(i=><div className={i.itemType === "combo" ? "adminComboOrderItem" : ""} key={i.lineId}><strong>{i.quantity} × {i.name}</strong>{i.itemType === "combo" && i.comboItems?.length ? <div className="adminComboChildren">{i.comboItems.map(child=><span key={child.productId}><b>{child.emoji} {child.name}</b><small>{comboChildOptions(child)}</small></span>)}</div> : <small>{orderItemOptions(i)}</small>}<b>{money(i.unitPrice * i.quantity)}</b></div>)}</div><div className="adminOrderMeta"><span><b>Payment</b>{o.payment}</span>{Boolean(o.giftCardAmount) && <span><b>Gift Card</b>{money(o.giftCardAmount || 0)} applied · {money(o.amountDue ?? Math.max(0, o.total - (o.giftCardAmount || 0)))} due</span>}{o.address && <span><b>Delivery address</b>{[o.address,o.apartment,o.city,o.zip].filter(Boolean).join(", ")}</span>}</div><div className="adminNote"><strong>Customer note</strong><p>{o.note||"No special note."}</p></div></ModalShell>; }
  ...
  if(modal.type==="product") { const p=entity as Product|undefined; const sku=p?.sku||generatedProductSku(db.products,db.categories,productCategoryId); return <ModalShell title={p?"Edit product":"New product"} subtitle="Menu item details and customization" close={close}><form onSubmit={submit} className="adminForm"><FormInput label="Product name" name="name" defaultValue={p?.name||""}/><FormInput label="Vietnamese name" name="vietnameseName" defaultValue={p?.vietnameseName||""}/><label>SKU code<input name="sku" value={sku} readOnly aria-readonly="true"/><small>Generated automatically from the category and next available number.</small></label><label>Category<select name="categoryId" value={productCategoryId} onChange={(event)=>setProductCategoryId(event.target.value)}>{db.categories.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select></label><FormInput label="Price" name="price" type="number" step="0.01" defaultValue={String(p?.price||0)}/><FormTextarea label="Description" name="description" defaultValue={p?.description||""}/><ImageUpload image={image||p?.image||""} setImage={setImage}/><fieldset className="adminChecklist wide"><legend>Customer Options</legend><Check name="allowIce" label="Allow Ice Level" checked={p?.allowIce??true}/><Check name="allowSugar" label="Allow Sugar Level" checked={p?.allowSugar??true}/><Check name="allowToppings" label="Allow Toppings" checked={p?.allowToppings??((p?.toppingIds.length||0)>0)}/></fieldset><fieldset className="adminChecklist wide"><legend>Available Toppings</legend>{db.toppings.map(t=><Check key={t.id} name="toppings" value={t.id} label={`${t.name} (+${money(t.price)})`} checked={p?.toppingIds.includes(t.id)}/>)}</fieldset><fieldset className="adminChecklist wide"><legend>Badges & Visibility</legend><Check name="bestSeller" label="Best Seller" checked={p?.bestSeller}/><Check name="mustTry" label="Must Try" checked={p?.mustTry}/><Check name="featured" label="Featured" checked={p?.featured}/><Check name="isNew" label="New" checked={p?.isNew}/><Check name="soldOut" label="Sold Out" checked={p?.soldOut}/><Check name="active" label="Published" checked={p?.active??true}/></fieldset><FormActions close={close}/></form></ModalShell>; }
  if(modal.type==="category") { const c=entity as Category|undefined; return <SimpleEntityForm title={c?"Edit category":"New category"} submit={submit} close={close}><FormInput label="Category name" name="name" defaultValue={c?.name||""}/><CategoryIconPicker value={c?.icon||"☕"}/><Check name="active" label="Active" checked={c?.active??true}/></SimpleEntityForm>; }
  if(modal.type==="topping") { const t=entity as Topping|undefined; return <ModalShell title={t?"Edit topping":"New topping"} subtitle="Quick setup" close={close}><form onSubmit={submit} className="adminForm compact"><FormInput label="Topping name" name="name" defaultValue={t?.name||""}/><FormInput label="Additional price" name="price" type="number" step="0.01" defaultValue={String(t?.price||0)}/><Check name="active" label="Active" checked={t?.active??true}/><FormActions close={close}/></form></ModalShell>; }
  if (modal.type === "combo") {
  ...
  const p=entity as Promotion|undefined; const date=new Date().toISOString().slice(0,10); return <ModalShell title={p?"Edit homepage slide":"New homepage slide"} subtitle="Homepage slider content" close={close}><form onSubmit={submit} className="adminForm" autoComplete="off"><label>Display style<select name="displayMode" value={promotionDisplayMode} onChange={event=>setPromotionDisplayMode(event.target.value as "designed"|"full_image")}><option value="designed">Designed layout</option><option value="full_image">Full advertisement image</option></select></label><FormInput label="Click destination" name="linkUrl" defaultValue={p?.linkUrl||"/menu"}/>{promotionDisplayMode==="designed"?<><FormInput label="Headline" name="title" defaultValue={p?.title||""}/><FormInput label="Eyebrow" name="eyebrow" defaultValue={p?.eyebrow||""}/><FormInput label="Price text" name="priceText" defaultValue={p?.priceText||""}/><FormTextarea label="Description" name="description" defaultValue={p?.description||""}/></>:<label>Image description <small>For accessibility only</small><input name="altText" defaultValue={p?.title||"Homepage advertisement"} autoComplete="off"/></label>}<FormInput label="Slide order" name="order" type="number" defaultValue={String(p?.order||db.promotions.length+1)}/><FormInput label="Start date" name="startDate" type="date" defaultValue={p?.startDate||date}/><FormInput label="End date" name="endDate" type="date" defaultValue={p?.endDate||date}/><ImageUpload kind="promotion" label="Desktop advertisement (1600 × 800)" image={image||p?.image||""} setImage={setImage}/><ImageUpload kind="promotion" label="Mobile advertisement (1080 × 1350)" image={mobileImage||p?.mobileImage||""} setImage={setMobileImage}/><Check name="active" label="Active within this date range" checked={p?.active??true}/><FormActions close={close}/></form></ModalShell>;
}
...
function AdminIcon({ name }: { name: AdminIconName }) {
  ...
  const paths: Record<AdminIconName, React.ReactNode> = {
    ...
    salespromotions: <><path d="M20 12 12 20 4 12 12 4l8 8Z"/><circle cx="9" cy="9" r="1"/><circle cx="15" cy="15" r="1"/><path d="m9 15 6-6"/></>,
    promotions: <><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m7 15 3-3 3 2 4-5"/></>,
    content: <><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a14 14 0 0 1 0 18M12 3a14 14 0 0 0 0 18"/></>,
    qrcode: <><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path d="M14 14h3v3h-3zM19 14h2v2M14 19h2v2M18 18h3v3"/></>,
