...
function AdminModal({ modal, db, close, update }: { modal: { type: string; id?: string }; db: DB; close: () => void; update: (d: DB, m?: string) => void }) {
  ...
  if(modal.type==="topping") { const t=entity as Topping|undefined; return <ModalShell title={t?"Edit topping":"New topping"} subtitle="Quick setup" close={close}><form onSubmit={submit} className="adminForm compact"><FormInput label="Topping name" name="name" defaultValue={t?.name||""}/><FormInput label="Additional price" name="price" type="number" step="0.01" defaultValue={String(t?.price||0)}/><ImageUpload kind="topping" label="Topping image" image={image||t?.image||""} setImage={setImage}/><Check name="active" label="Active" checked={t?.active??true}/><FormActions close={close}/></form></ModalShell>; }
  if (modal.type === "combo") {
