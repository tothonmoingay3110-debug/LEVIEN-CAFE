...
function CustomerView({ data }: { data: BusinessIntelligenceData }) {
  ...
  return <>
    <section className="intelligenceSegments">{data.segments.map((segment) => <div className={`intelligenceSegment segment-${segment.name.toLowerCase()}`} key={segment.name} title={segment.name === "VIP" ? "VIP: at least 15 completed orders or $300 lifetime spend" : segment.name === "Loyal" ? "Loyal: at least 5 completed orders" : segment.name === "Returning" ? "Returning: at least 2 completed orders" : "New: first completed order"}><span>{segment.name}{segment.name === "VIP" ? " ⓘ" : ""}</span><strong>{segment.customers}</strong><small>{money(segment.revenue)} revenue</small>{segment.name === "VIP" ? <em>15+ orders or $300+ lifetime spend</em> : null}</div>)}</section>
