  ...
  return (
    <>
      <article className="productCard">
        ...
        <div className="productBody"><div className="productMeta">{product.vietnameseName ? <span>{product.vietnameseName}</span> : <span aria-hidden="true" />}<strong>${product.price.toFixed(2)}</strong></div><h3>{product.name}</h3><p>{product.description}</p>{offers.length>0&&<div className="productPromotionSummary"><strong>{offers[0].badge_text}</strong><span>{offers[0].description||offers[0].name}</span></div>}<button className="productAddButton" disabled={product.soldOut} onClick={add}>{product.soldOut ? "Sold Out" : hasCustomization ? "Customize & Add" : "Add to Order"}</button></div>
      </article>
